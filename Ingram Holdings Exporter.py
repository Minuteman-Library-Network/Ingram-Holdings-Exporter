...
def marcwriter(file):
  ...
  for fname in os.listdir("."):
    if os.path.isfile(fname) and fname.startswith("holdings"):
      os.remove(fname)
  
  #create an output file for the created MARC records (wb means 'write binary')  
  outputfile = open('holdings{}.mrc'.format(date.today()), 'wb')
...
def runquery():
    ...
    cursor = conn.cursor()
    cursor.execute(open("Ingram Holdings Exporter.sql","r").read())
...
#Enter Ingram's host, user and password in the following 2 lines
with FTP("") as ftp:
    ftp.login(user="", passwd = "")
    fp = open('holdings{}.mrc'.format(date.today()), 'rb')
    ftp.storbinary('STOR %s' % os.path.basename('holdings{}.mrc'.format(date.today())), fp)
    fp.close()
    ftp.quit()
